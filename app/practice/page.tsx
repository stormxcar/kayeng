...
export default function PracticePage(){
  ...
  return <PageShell eyebrow="PRACTICE CENTER" title="Trung tâm luyện tập">
    ...
    {items.length>0&&<section className="live-practice"><header><div><small>ACTIVITY ENGINE</small><h2>Bài luyện từ khóa học của bạn</h2></div><span>{items.length} hoạt động đang có dữ liệu</span></header><div>{items.map(item=><Link href={`/lesson?id=${item.lessons.id}`} key={item.id}><Sparkles/><div><small>{item.lessons?.title} • {item.activity_type}</small><h3>{item.title}</h3><p>{item.instructions}</p></div><ArrowRight/></Link>)}</div></section>}
