      ...
      {!loading && !user ? <div className="empty-state"><h2>Bạn chưa đăng nhập</h2><Link href="/">Đăng nhập tại trang chủ</Link></div> : profile && (
        <div className="profile-layout">
          <aside className="profile-summary">
            <div className="profile-avatar">{(profile.display_name || "K").slice(0, 1).toUpperCase()}</div>
          ...
          <form className="profile-form" onSubmit={save}>
            <div className="form-section"><p className="section-kicker">THÔNG TIN CÁ NHÂN</p><div className="form-grid"><label>Tên hiển thị<input name="displayName" defaultValue={profile.display_name || ""} /></label><label>Nghề nghiệp<input name="occupation" defaultValue={profile.occupation || ""} /></label></div></div>
